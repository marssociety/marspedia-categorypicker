...
ve.ui.MWCategoryPopupWidget = function VeUiMWCategoryPopupWidget( config ) {
	...
	/*  @TMS - CommentedOut this.sortKeyInput = new OO.ui.TextInputWidget();
	this.sortKeyField = new OO.ui.FieldLayout( this.sortKeyInput, {
		align: 'top',
		label: ve.msg( 'visualeditor-dialog-meta-categories-sortkey-label' )
	} );
	*/
	this.$sortKeyForm = $( '<form>' )// @TMS - CommentedOut .addClass( 've-ui-mwCategoryPopupWidget-sortKeyForm' )
		.append( ); // @TMS - CommentedOut this.sortKeyField.$element );
	...
	this.toggle( false );
	this.$title
		.addClass( 've-ui-mwCategoryPopupWidget-title') //  @TMS - CommentedOut oo-ui-icon-tag' )
		.text( 'Remove Category'); // @TMS - CommentedOut ve.msg( 'visualeditor-dialog-meta-categories-category' ) );
...
ve.ui.MWCategoryPopupWidget.prototype.onToggle = function ( show ) {
	...
	//  @TMS - CommentedOut newSortkey = this.sortKeyInput.$input.val();
	// @TMS - CommentedOut if ( !this.removed && newSortkey !== ( this.origSortkey || '' ) ) {
...
ve.ui.MWCategoryPopupWidget.prototype.loadCategoryIntoPopup = function ( item ) {
	...
	if ( item.isHidden ) {
		...
		// @TMS - CommentedOut this.$hiddenStatus.text( 'Click the Trash Can Icon to remove the category'); //ve.msg( 'visualeditor-dialog-meta-categories-missing' ) );
	} else {
		this.$hiddenStatus.empty();
	}
	// @TMS - CommentedOut this.sortKeyInput.$input.val( item.sortKey );
};
